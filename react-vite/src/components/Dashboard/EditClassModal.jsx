...
import { editClass } from "../../redux/class";

function EditClassModal({classEdit}) {
  const dispatch = useDispatch();
  const [name, setName] = useState(classEdit.name);
  const [subject, setSubject] = useState(classEdit.subject);
  const [grade, setGrade] = useState(classEdit.grade);
  const [room, setRoom] = useState(classEdit.room);
  const [period, setPeriod] = useState(classEdit.period);
  ...
  return (
    <div className='formCon'>
      <h1 className='inputTitle'>Edit Class</h1>
